...
class DateManager {
  ...
  getFutureDate(options) {
    return new Date(
      this.#today.getFullYear() + options.years,
      this.#today.getMonth() + options.months,
      this.#today.getDate() + options.days,
      0,
      0,
      0,
      0
  ...
  getProperTodoDate(date) {
    ...
    if (
      isNaN(date.getTime()) ||
      Math.trunc(date.getFullYear() / 1000) !==
        Math.trunc(this.#today.getFullYear() / 1000) ||
      date < this.#today
    ) {
      return DateManager.noDateValue;
    }
