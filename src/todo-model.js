...
export default class TodoModel extends AbstractTodoModel {
  ...
  assign(obj) {
    if (obj) {
      this.#projects[TodoModel.#specialProjects.inbox] = new TodoProject(
        "Inbox"
      ).assign(obj.inbox);
      let i = TodoModel.getSpecialProjectsLength();
      obj.projects.forEach((project) => {
        this.#projects[i++] = new TodoProject().assign(project);
      });
  ...
  addProject(projectName) {
    ...
    // announce that we didn't add the project since a project with this name already exists
      ...
      this.publish(TOPICS.projectAdded, {
        errorMessage: `A project with the name ${projectName} is already exists`,
      });
    }
  }
  ...
  sortProject(sortName) {
    if (this.#currentProject.sort(sortName)) {
      this.publish(TOPICS.projectSorted, {
        sortName,
  ...
  addTodo(todo, project = this.#currentProject) {
    ...
    if (
      ...
      specialProject === this.#currentProject
    ) {
    ...
    // the current project, that is actually today's project, since it will not be a valid date for this project.
      ...
      this.publish(TOPICS.todoAdded, {
        todo,
        position,
        isAddedToAnotherProject: true,
      });
    }
  }
  ...
  isCurrentProjectSpecial() {
    return (
      this.#currentProject ===
        this.#projects[TodoModel.#specialProjects.today] ||
      this.#currentProject ===
        this.#projects[TodoModel.#specialProjects.nextSevenDays]
    );
  }
  ...
  set currentProject(projectName) {
    ...
    if (project) {
      this.#currentProject = project;
      this.publish(TOPICS.currentProjectChanged, {
  ...
  updateSpecialProjects(op, dueDate, todoData) {
    if (dueDate instanceof Date) {
      if (
        this.#projects[TodoModel.#specialProjects.today].dateRange.includes(
          dueDate
        )
      ) {
        ...
        this.#projects[
          TodoModel.#specialProjects.nextSevenDays
        ].dateRange.includes(dueDate)
      ) {
  ...
  isSpecialProject(projectName) {
    return (
      projectName === this.#projects[TodoModel.#specialProjects.today].name ||
      projectName ===
        this.#projects[TodoModel.#specialProjects.nextSevenDays].name
    );
  }
  ...
  changeProjectName(projectName, updatedProjectName) {
    if (!this.getProjectByName(updatedProjectName)) {
      ...
      pubsub.publish(TOPICS.projectNameChanged, {
        projectName,
        updatedProjectName,
      });
    }
