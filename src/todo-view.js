...
export default class TodoView extends AbstractSubscriber {
  ...
  createView() {
    ...
    this.#editTodoFormContainer = this.#todos.querySelector(
      ".edit-todo-list-item"
    );
    ...
    this.#projectContentHeader = document.querySelector(
      ".project-content-header"
    );

    this.#currentProject =
      this.#constantProjects.firstElementChild.nextElementSibling;
    ...
    this.#projectContent.querySelector("h3").textContent =
      this.#todoModel.currentProject.name;
  ...
  populateProjects(projectsNames) {
    projectsNames.forEach((projectName) => {
      this.#customProjects.append(this.#createProjectListItem(projectName));
    });
  }
  ...
  onTodoAdded(data) {
    if (data.todo instanceof Todo) {
      if (!data.isAddedToAnotherProject) {
        this.#todos.children[data.position].before(
          this.#createTodoListItem(data.todo)
        );
      } else {
  ...
  #injectTodoDetails(todoListItem, todo) {
    ...
    todoListItem.querySelector(".todo-list-item-title").textContent =
      todo.title;
    todoListItem.querySelector(".todo-list-item-date").textContent =
      dateManager.toDateString(todo.dueDate);
    todoListItem
      .querySelector(".todo-list-item-priority")
      .classList.add(this.#getPriorityClass(todo.priority));
  }
  ...
  #isChanged(properties, updatedProperties) {
    return (
      !dateManager.equals(properties.date, updatedProperties.date) ||
  ...
  #updateTodo(todoListItem) {
    let todoProperties = this.#todoController.getTodoProperties(
      todoListItem.dataset.id
    );

    if (!todoProperties) return;
    ...
    this.#editTodoForm.elements.submit.onclick = function (event) {
      ...
      if (title) {
        let updatedProperties = {
          ...
          date: dateManager.resetHours(
            dateManager.getProperTodoDate(
              this.#editTodoForm.elements.date.valueAsDate
            )
          ),
  ...
  #handleErrorMsg() {
    ...
    // closing the form.
    if (
      this.#editTodoForm.elements.title.nextElementSibling !==
      this.#editTodoForm.elements.description
    ) {
  ...
  #populateEditTodoForm(todoProperties) {
    ...
    this.#editTodoForm.elements.date.value = dateManager.toInputDateFormat(
      todoProperties.date
    );
  ...
  onPriorityClick() {
    ...
    priorityList.onclick = function (event) {
      ...
      if (!priority || priority.classList.contains("chosen")) return;
  ...
  onCurrentProjectChanged(data) {
    if (data instanceof TodoProject) {
      this.#handleProjectSelectionMenu(
        document.querySelector("li.current-project > span").textContent,
        data.name
      );
  ...
  #setSortSelection(sortName = this.#todoModel.getCurrentProjectSortName()) {
    let options =
      this.#projectContentHeader.querySelector(".sort-project-menu").options;
  ...
  onClickChangeCurrentProject(event) {
    ...
    if (project && this.#currentProject !== project) {
      this.#todoController.changeCurrentProject(
        project.querySelector("span").textContent
      );
    }
  }
  ...
  #addListeners() {
    ...
    this.#addProjectBtn.addEventListener(
      "click",
      this.onClickAddProject.bind(this)
    );
    this.#projects.addEventListener(
      "click",
      this.onClickChangeCurrentProject.bind(this)
    );
    ...
    this.#projectSelectionMenu.addEventListener(
      "change",
      this.onChangeProjectSelection.bind(this)
    );
    this.#customProjects.addEventListener(
      "click",
      this.onCustomProjectsClick.bind(this)
    );
    this.#projectContentHeader.addEventListener(
      "change",
      this.onProjectContentHeaderChange.bind(this)
    );
  }
  ...
  #editProjectName(projectListItem) {
    ...
    form.onsubmit = function (event) {
      ...
      if (value) {
        this.#todoController.changeProjectName(
          projectListItem.firstElementChild.textContent,
          value
        );
      }
  ...
  onChangeProjectSelection(event) {
    if (this.#hiddenTodo) {
      this.#projectSelectionMenu.form.elements.submit.onclick =
        this.#projectSelectionMenu.form.elements.cancel.onclick = null;
      this.#todoController.moveTodoToProject(
        this.#hiddenTodo.dataset.id,
        event.target.value
      );
    }
  }
  ...
  #showEditTodoForm() {
    ...
    this.#addTodoListItem.style.display = "none";
    this.#projectContentHeader.querySelector(
      ".sort-project-menu"
    ).disabled = true;
  }
  ...
  #hideEditTodoForm() {
    ...
    for (let priority of this.#editTodoForm.querySelector(
  ...
  #createProjectListItem(projectName) {
    ...
    let projectSettings = document
      .querySelector("#project-settings-template")
